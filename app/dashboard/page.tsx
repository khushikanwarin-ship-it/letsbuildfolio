              ...
              {featured.length > 0 && !q && stream === "All" && type === "All" && (
                <div className="mb-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    {featured.map((opp, i) => (
                      <OppCard key={opp.id} opp={opp} i={i} saved={saved.has(opp.id)} onSave={() => toggleSave(opp.id)} expanded={expanded === opp.id} onExpand={() => setExpanded(expanded === opp.id ? null : opp.id)} />
                    ))}
                  </div>
                </div>
              )}
...
function OppCard({ opp, i, saved, onSave, expanded, onExpand }: {
